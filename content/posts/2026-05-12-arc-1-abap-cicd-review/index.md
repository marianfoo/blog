...
Series note: This post is part of my [AI ABAP development series](/tags/ai-abap-development-series/). In the previous posts I introduced [ARC-1](https://github.com/arc-mcp/arc-1), then used it from BTP, Copilot Studio, Joule Studio, and a SEGW to RAP migration.
...
So I created a small sample repository for this pattern: [github.com/arc-mcp/arc-1-abap-cicd-review](https://github.com/arc-mcp/arc-1-abap-cicd-review).
...
The repo has six small workflow files: [`pr.yml`](https://github.com/arc-mcp/arc-1-abap-cicd-review/blob/main/.github/workflows/pr.yml), [`sap-tests.yml`](https://github.com/arc-mcp/arc-1-abap-cicd-review/blob/main/.github/workflows/sap-tests.yml), [`copilot-review-trigger.yml`](https://github.com/arc-mcp/arc-1-abap-cicd-review/blob/main/.github/workflows/copilot-review-trigger.yml), [`claude-review-trigger.yml`](https://github.com/arc-mcp/arc-1-abap-cicd-review/blob/main/.github/workflows/claude-review-trigger.yml), [`sap-dump-triage.yml`](https://github.com/arc-mcp/arc-1-abap-cicd-review/blob/main/.github/workflows/sap-dump-triage.yml), and [`sap-dump-deep-dive.yml`](https://github.com/arc-mcp/arc-1-abap-cicd-review/blob/main/.github/workflows/sap-dump-deep-dive.yml). ARC-1 is the common backend. GitHub Actions is only the automation surface.
...
[`sap-tests.yml`](https://github.com/arc-mcp/arc-1-abap-cicd-review/blob/main/.github/workflows/sap-tests.yml) runs on every PR that touches `src/`. It parses the abapGit filenames, maps them back to ABAP object types, and then calls ARC-1 directly from Bash. There is no model involved in this part.

For changed classes it runs ABAP Unit through [`SAPDiagnose`](https://docs.arc-1-mcp.com/tools/#sapdiagnose) with `action="unittest"`. For changed classes, interfaces, programs, and function groups it runs ATC through [`SAPDiagnose`](https://docs.arc-1-mcp.com/tools/#sapdiagnose) with `action="atc"`. The result is posted as a sticky PR comment, and line-specific ATC findings are posted inline on the Files Changed tab when GitHub can attach them to the diff.
...
The clean demo is [PR #20](https://github.com/arc-mcp/arc-1-abap-cicd-review/pull/20). It adds one more unit test to [`ZCL_ARC1_TEST_HELPERS`](https://github.com/arc-mcp/arc-1-abap-cicd-review/blob/main/src/zcl_arc1_test_helpers.clas.abap). The workflow [reports 7 of 7 ABAP Unit tests passing](https://github.com/arc-mcp/arc-1-abap-cicd-review/pull/20#issuecomment-4432679538) and one non-blocking P3 ATC warning.
...
In the sample [PR #14](https://github.com/arc-mcp/arc-1-abap-cicd-review/pull/14), Copilot Code Review did catch the [direct `SELECT` from `MARA`](https://github.com/arc-mcp/arc-1-abap-cicd-review/pull/14#discussion_r3226642123) as a Clean Core hotspot. That is a good result.
...
In PR #14, the decisive ARC-1 call was a [`SAPRead`](https://docs.arc-1-mcp.com/tools/#sapread) of the activated `ZCL_ARC1_TASK_SERVICE~LIST_TASKS` method from SAP. Claude then posted the [key finding](https://github.com/arc-mcp/arc-1-abap-cicd-review/pull/14#discussion_r3228270334): the `MARA` block is not only a direct SAP table access, it also changes the meaning of `list_tasks`. A task service should not silently return no tasks because the material master has no finished product.
...
The main demo is [PR #14](https://github.com/arc-mcp/arc-1-abap-cicd-review/pull/14). It contains one harmless refactoring and one deliberately bad preflight inside `ZCL_ARC1_TASK_SERVICE~LIST_TASKS`:
...
- [Copilot Coding Agent with ARC-1](https://github.com/arc-mcp/arc-1-abap-cicd-review/pull/14#issuecomment-4430841521) can combine diff review with live SAP reads, syntax, navigation, and release checks.
- [Claude with ARC-1](https://github.com/arc-mcp/arc-1-abap-cicd-review/pull/14#discussion_r3228270334) posts a focused review with inline comments, suggestion blocks, and the SAP drift check.

The separate SAP test gate example is [PR #20](https://github.com/arc-mcp/arc-1-abap-cicd-review/pull/20): one extra test method, 7 of 7 ABAP Unit tests green, and a single non-blocking P3 ATC finding.
...
This is now verified in PR #14. [Copilot Code Review](https://github.com/arc-mcp/arc-1-abap-cicd-review/pull/14#discussion_r3226642123) and [Claude](https://github.com/arc-mcp/arc-1-abap-cicd-review/pull/14#discussion_r3228270334) both suggested deleting the `MARA` preflight block. Claude's version adds the SAP-side evidence on top: [`SAPRead`](https://docs.arc-1-mcp.com/tools/#sapread) confirmed that this block is not part of the currently activated method, so the review can separate "new in this PR" from "already active in SAP".
...
The [`sap-dump-triage.yml`](https://github.com/arc-mcp/arc-1-abap-cicd-review/blob/main/.github/workflows/sap-dump-triage.yml) workflow asks ARC-1 for recent ST22 dumps through [`SAPDiagnose`](https://docs.arc-1-mcp.com/tools/#sapdiagnose). It can run on a schedule, deduplicates dumps against existing GitHub issues by storing the dump ID in an HTML comment marker, and creates one issue for each new dump with metadata, a short Claude triage, and an urgency label.

For the sample repo, the cron is disabled so the issue list stays stable. In a real setup, I would turn it on so GitHub becomes a lightweight watch list for new dumps instead of waiting until someone reports the same runtime error again. The current examples are visible through the [`sap:dump` label](https://github.com/arc-mcp/arc-1-abap-cicd-review/issues?q=is%3Aissue+label%3Asap%3Adump).
...
The second stage is the [`sap-dump-deep-dive.yml`](https://github.com/arc-mcp/arc-1-abap-cicd-review/blob/main/.github/workflows/sap-dump-deep-dive.yml) workflow. A human applies the `dump:investigate` label to a dump issue. The workflow extracts the dump ID, reads the full dump through ARC-1, reads the failing ABAP source, and posts one investigation comment.

The best demo is [issue #15](https://github.com/arc-mcp/arc-1-abap-cicd-review/issues/15), an `OBJECTS_OBJREF_NOT_ASSIGNED_NO` dump in `CL_ENH_ADT_ENHO_OBJ_PERSIST`.
...
The [deep-dive comment](https://github.com/arc-mcp/arc-1-abap-cicd-review/issues/15#issuecomment-4431477985) then updates the labels from `needs-triage` to `dump:investigated`. So GitHub becomes the lightweight operations board, while SAP remains the source of truth.
...
The next obvious step is the post-merge path. After a PR is merged to `main`, a GitHub workflow could call ARC-1 with a separate git-scoped key and run an abapGit pull through [`SAPGit`](https://docs.arc-1-mcp.com/tools/#sapgit) for the package in the SAP system.
...
- [Sample repository: arc-1-abap-cicd-review](https://github.com/arc-mcp/arc-1-abap-cicd-review)
- [Demo PR #14: multi-engine review with suggestion blocks](https://github.com/arc-mcp/arc-1-abap-cicd-review/pull/14)
- [Demo PR #20: SAP Unit and ATC gate](https://github.com/arc-mcp/arc-1-abap-cicd-review/pull/20)
- [Dump issue #15: deep-dive investigation](https://github.com/arc-mcp/arc-1-abap-cicd-review/issues/15)
- [Dump tracker issues](https://github.com/arc-mcp/arc-1-abap-cicd-review/issues?q=is%3Aissue+label%3Asap%3Adump)
